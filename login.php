<?php
// Evita errores de "Headers Already Sent" al guardar el output en búfer
ob_start(); 
session_start();

// --- Bloque 1: Validación GET de usuario activo/inactivo ---
$usuarios = [
    1 => ['estado' => 'activo'],
    2 => ['estado' => 'inactivo'],
];

// Solo ejecutamos este bloque si el parámetro realmente viene en la URL
...
if (!empty($_POST)) {
    if (empty($_POST['nom_usu']) || empty($_POST['contr_usu'])) {
        ...
        // Convertimos a minúsculas para contrarrestar el toUpperCase() del HTML y coincidir con la DB en Linux
...
<head>
    ...
    <style media="screen">
        *, *:before, *:after { padding: 0; margin: 0; box-sizing: border-box; }
        ...
        .shape:first-child { background: linear-gradient(#1876ad, #a623f6); left: -80px; top: -80px; }
        .shape:last-child { background:linear-gradient(to right, #ff2ff0ab, #f01940); right: -30px; bottom: -80px; }
        form { height: auto; min-height: 520px; width: 400px; background-color: rgba(255,255,255,0.13); position: absolute; transform: translate(-50%,-50%); top: 50%; left: 50%; border-radius: 10px; backdrop-filter: blur(10px); border: 2px solid rgba(255,255,255,0.1); box-shadow: 0 0 40px rgba(8,7,16,0.6); padding: 50px 35px; }
        ...
        .error-box { background-color: rgba(255, 0, 0, 0.2); color: #ff6b6b; padding: 12px; border-radius: 5px; text-align: center; margin-top: 20px; font-size: 14px; border: 1px solid rgba(255, 0, 0, 0.3); }
    </style>
</head>
<body>
