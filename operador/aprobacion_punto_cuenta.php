...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['nrp_pta']) && isset($_POST['anio'])) {
        ...
        if (!preg_match('/^[1-9]\d{0,4}$/', $nropto)) {
            echo "<script>alert('❌ Número de solicitud inválido.');</script>";
            $conn->close();
            exit;
        }
        ...
        if ($anio_int === false || $anio_int < 1562 || $anio_int > 2055) {
            echo "<script>alert('❌ Año inválido.');</script>";
            $conn->close();
            exit;
        }
        ...
        if (!empty($results)) {
            ...
            foreach ($_SESSION['puntoctaes'] as $key => $puntocta) {
                if ($puntocta['nrp_pta'] === $results[0]['nrp_pta'] && 
                    ...
                    $exists = true;
                    echo "<script>alert('✅ Punto de cuenta eliminado.');</script>";
                    break;
                }
            }
            if (!$exists) {
    ...
    // ==========================================
        ...
        $_SESSION['puntoctaes'] = [];
        echo "<script>alert('🔄 Restaurado.');</script>";
    ...
    // ==========================================
        ...
        foreach ($_SESSION['puntoctaes'] as $key => $puntocta) {
            if ($puntocta['nrp_pta'] === $nrp_pta) {
                unset($_SESSION['puntoctaes'][$key]);
                echo "<script>alert('🗑️ Desconsultado.');</script>";
                break;
            }
        }
        
    // ==========================================
    // 4. APROBACIÓN - SOLO DESCRIPCIÓN
    // ==========================================
    } elseif (isset($_POST['monto'])) {
        $monto = trim($_POST['monto']);
        
        // 🛡️ VALIDACIÓN SERVIDOR: Descripción
        if (empty($monto) || strlen($monto) < 3 || strlen($monto) > 200 || preg_match('/[<>]/', $monto)) {
            echo "<script>alert('❌ Descripción inválida.\nDebe tener entre 3 y 200 caracteres, sin símbolos < o >.');</script>";
            $conn->close();
            exit;
        }
        
        // Guardar en sesión para enviar a pre_aprobacion.php
        $_SESSION['monto'] = $monto;
        
        // Redirigir para evitar reenvío de formulario
        header("Location: pre_aprobacion.php");
        exit;
    }
}
$conn->close();
...
<body class="light-theme">
    ...
    <div class="container">
        ...
        <form method="POST" id="consultaForm" novalidate>
            ...
            <div class="form-row">
                <div class="col-md-6">
                    <label for="nrp_pta">Número de punto de cuenta:</label>
                    <input 
                        type="text" 
                        id="nrp_pta" 
                        name="nrp_pta" 
                ...
                <div class="col-md-6">
                    ...
                    <input 
                        ...
                        required
                        pattern="^[1-9]\d{3}$"
                        title="Año entre 1562 y 2055, 4 dígitos sin ceros iniciales"
                        inputmode="numeric" 
                        autocomplete="off"
                        maxlength="4"
                    >
                    <small id="anio_error" class="error-message">
                        ⚠️ 4 dígitos numéricos entre 1562 y 2055
                    </small>
                </div>
        ...
        <!-- FORMULARIO DE APROBACIÓN (SOLO DESCRIPCIÓN) -->
        <form method="POST" action="pre_aprobacion.php" id="aprobacionForm" novalidate>
            ...
            <center><h2>Registrar Descripción</h2></center>
            <div class="form-row" style="width: auto; justify-content: center;">
                <div>
                    ...
                    <input 
                        ...
                        style="width: 350px;"
                    >
                    <small id="monto_error" class="error-message">
                        ⚠️ Mínimo 3 caracteres alfanuméricos
                    </small>
                </div>
            </div>

            <center><button type="submit" id="btnAprobar" disabled>Realizar aprobación</button></center>
        </form>
    </div>
...
<script>
    ...
    // ==========================================
    // VALIDACIÓN CAMPO DESCRIPCIÓN (MONTO)
    // ==========================================
    function validateMonto(input) {
        const errorEl = document.getElementById('monto_error');
        const value = input.value.trim();
    ...
    // ==========================================
    // CONTROL DEL BOTÓN APROBAR (SOLO MONTO)
    // ==========================================
    function updateAprobarButton() {
        const btn = document.getElementById('btnAprobar');
        if (!btn) return;
        
        const montoValid = validateMonto(document.getElementById('monto'));
        btn.disabled = !montoValid;
    }
