...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Verifica si la variable 'puntoctaes' está definida
    ...
    if ($conn->query($sql_update_numsol) === TRUE) {
       ...
       if ($result_numsol->num_rows > 0) {
           while ($row_numsol = $result_numsol->fetch_assoc()) {
               $nrp_pta_array[] = $row_numsol['nrp_pta']; // Agregar cada nrp_pta al arreglo
           }
       } else {
           $mensaje .= "Error al buscar número de solicitud: " . $conn->error;
       }
       
       // Unir los elementos del arreglo en una cadena separada por comas
       $nrp_pta_string = implode(',', $nrp_pta_array);
   
   
       echo $nrp_pta_string;
       
       $sql_update_numsol = "UPDATE `solicitud` SET `observa`='$monto', `fecpago`= '$fechanueva',`estatus`='3' , `cedusu` = '$id_usu'  WHERE `ptocuenta` IN ($nrp_pta_string);"; // Incrementar numsol
    if ($conn->query($sql_update_numsol) === TRUE) {
