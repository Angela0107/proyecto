...
// Verificar si 'solicitudes' fue enviada por POST
if (!isset($_SESSION['solicitudes']) || empty($_SESSION['solicitudes'])) {
    header('Location: error_punto.php');
    exit;
}


$monto = $_SESSION['monto']; // Asegúrate de que estas variables estén definidas
$fechanueva = $_SESSION['fecha'];
$solicitudes = $_SESSION['solicitudes'];
$nomprov = $_SESSION['nombre_propio'];
$nacio = $_SESSION['nacio_propio'];
$cedprov = $_SESSION['Cedula_propio'];
$nrocuenta = $_SESSION['numero_cuenta_propio'];
$nombanco = $_SESSION['codban_propio'];
$tipcuenta = $_SESSION['tipcuenta_propio'];
...
<form action="guardar_punto_cuenta.php" method="post">
    ...
    <input type="hidden" id="banco" name="banco" value="<?php echo $nombanco; ?>"> 
    <input type="hidden" id="nrocta" name="nrocta" value="<?php echo $nrocuenta; ?>"> 
    <input type="hidden" id="tipcuenta" name="tipcuenta" value="<?php echo $tipcuenta; ?>"> 
    <input type="hidden" id="nomprov" name="nomprov" value="<?php echo $nomprov; ?>"> 
    <input type="hidden" id="nacio" name="nacio" value="<?php echo $nacio; ?>"> 
    <input type="hidden" id="cedprov" name="cedprov" value="<?php echo $cedprov; ?>"> 
    <input type="hidden" id="tipo" name="tipo" value="<?php echo $tipcuenta; ?>"> 
        ...
        <div class="form-row">
            ...
            <div class="form-row">
                <div class="borderedasunto bordered">
                    <textarea name="asunto" id="asunto" class="texto_asunto">Se somete a consideración y aprobación del Ciudadano Alcalde <strong>Dr. SILFREDO ZAMBRANO</strong>, la cancelación de ayudas económicas por concepto de <strong> <?php echo implode(", ", $descripciones); ?></strong> por un monto de <strong><?php echo strtoupper($monto_texto); ?></strong> con cero Centimos de BS (BS <?php echo $monto_formateado ?>), el cual se le será cancelado al Banco <?php echo $nombanco ?> Cuenta <?php echo $tipcuenta ?> N° <strong><?php echo $nrocuenta ?></strong> a Nombre de <strong><?php echo $nomprov ?> (<?php echo $nacio ?>-<?php echo $cedprov ?>)</strong> para los beneficiarios que se mencionan a continuación.</textarea>
                </div>
            </div>
